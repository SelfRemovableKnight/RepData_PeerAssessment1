...
The mean number of steps per day is `r format(step_mean, scientific=FALSE)`. 
...
The 5-minute time interval with the highest average number of steps is the one starting at `r best_interval_time `.
...
There are `r number_of_NAs` missing "steps" values in the data set, out of `r length(activity_data$steps)` observations.
...
After this correction, the mean number of steps per day is `r format(normalized_step_mean, scientific=FALSE)`. The median is `r format(normalized_step_median, scientific=FALSE)`. 
